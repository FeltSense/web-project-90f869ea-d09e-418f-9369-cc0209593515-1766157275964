        ...
        <div className="grid lg:grid-cols-2 gap-8 sm:gap-12 items-center">
          ...
          >
            ...
            >
              ...
              <span className="text-sm font-semibold text-cyan-200">✨ Trusted by 100K+ Home Cooks • 4.9★ Rating</span>
            </motion.div>

            <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold text-white mb-6 leading-tight">
              Your Personal AI Chef <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-300 via-emerald-300 to-blue-300">Dinner Solved in Seconds</span>
            </h1>
            <p className="text-base sm:text-lg md:text-xl text-slate-200 mb-8 leading-relaxed font-medium">
              Stop wasting time deciding what to cook. RecipeAI generates unlimited personalized recipes in seconds based on what you have, what you love, and your budget. <span className="font-bold text-emerald-300">Save 10+ hours weekly</span> on meal planning, <span className="font-bold text-cyan-300">cut grocery bills by 30%</span>, <span className="font-bold text-blue-300">reduce food waste by 40%</span>, and <span className="font-bold text-emerald-300">cook restaurant-quality meals</span> every night. <span className="font-bold text-emerald-300">Join 100K+ home cooks—start free today, no credit card needed.</span>
            </p>
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 w-full sm:w-auto">
              <motion.button
                className="px-8 sm:px-12 py-4 sm:py-5 bg-gradient-to-r from-cyan-400 via-emerald-400 to-blue-400 text-slate-950 font-bold rounded-lg sm:rounded-xl hover:from-cyan-300 hover:via-emerald-300 hover:to-blue-300 transition shadow-lg shadow-cyan-400/60 hover:shadow-cyan-400/90 flex items-center justify-center gap-2 group text-base sm:text-lg hover:scale-110 active:scale-95 relative overflow-hidden before:absolute before:inset-0 before:bg-white/30 before:opacity-0 hover:before:opacity-100 before:transition-opacity animate-pulse hover:animate-none"
                whileHover={{ scale: 1.1 }}
                whileTap={{ scale: 0.95 }}
              >
                <span className="relative z-10 flex items-center gap-2">
                  🚀 Get Started Free
                  <ArrowRight className="w-4 sm:w-5 h-4 sm:h-5 group-hover:translate-x-2 transition" />
                </span>
              </motion.button>
              <motion.button
                className="px-6 sm:px-10 py-4 sm:py-5 border-2 border-emerald-400/80 text-white font-semibold rounded-lg sm:rounded-xl hover:bg-emerald-500/40 hover:border-emerald-200 transition flex items-center justify-center gap-2 text-base sm:text-lg hover:scale-110 backdrop-blur-sm bg-emerald-500/20 active:scale-95 relative overflow-hidden before:absolute before:inset-0 before:bg-emerald-400/20 before:opacity-0 hover:before:opacity-100 before:transition-opacity shadow-lg shadow-emerald-500/50 hover:shadow-emerald-500/70"
                whileHover={{ scale: 1.1 }}
                whileTap={{ scale: 0.95 }}
              >
                <span className="relative z-10 flex items-center gap-2">
